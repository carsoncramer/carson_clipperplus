...
void CLIPPER::findDenseClique(const Eigen::VectorXd& u0)
{
  ...
  if (idxD.sum() > 0) {
    ...
    d = (num.array() / den.array()).minCoeff();    
      ...
      // std::cout << "clipper: M_:\n" << M_ << std::endl;
      // std::cout << "clipper: M_.selfadjointView<Eigen::Upper>():\n" << M_.selfadjointView<Eigen::Upper>() << std::endl; 
    #endif
  }
  #ifdef DEBUG_OPTIM
...
void CLIPPER::findMaximalClique(const Eigen::VectorXd& u0)
{
  ...
  for (i=0; i<params_.maxoliters; ++i) { // outerloop
    ...
    for (j=0; j<params_.maxiniters; ++j) { // innerloop                        
      ...
      // const Eigen::VectorXi idxA = ( ((gradF.array() < -params_.eps) && ( u.array() > params_.eps)) || (gradF.array() > params_.eps) ).cast<int>();
      ...
      if (idxA.sum() > 0) {
        ...
        // std::cout <<  "clipper: chose smallest alpha" << std::endl;
      } else {
        ...
        // std::cout <<  "clipper: chose power 3 alpha" << std::endl;
      }

      // Backtracking line search on gradient ascent
      double Fnew = 0, deltaF = 0;
      for (k=0; k<params_.maxlsiters; ++k) {
        // std::cout << "clipper: alpha: " << alpha << std::endl;
        // std::cout << "clipper: u: "; for (int ii=0; ii<u.size(); ++ii) {std::cout<<u(ii)<< " ";} std::cout << std::endl; 
        // std::cout << "clipper: gradF: "; for (int ii=0; ii<gradF.size(); ++ii) {std::cout<<gradF(ii)<< " ";} std::cout << std::endl; 
        ...
        // std::cout << "clipper: unew: "; for (int ii=0; ii<unew.size(); ++ii) {std::cout<<unew(ii)<< " ";} std::cout << std::endl; 
        // std::cout << "clipper: gradFnew (bef proj): "; for (int ii=0; ii<gradFnew.size(); ++ii) {std::cout<<gradFnew(ii)<< " ";} std::cout << std::endl; 
        
        gradFnew = gradFnew - Fnew * unew;            // project gradient

        deltaF = Fnew - F;                            // change in objective value

        // std::cout << "clipper: gradFnew: "; for (int ii=0; ii<gradFnew.size(); ++ii) {std::cout<<gradFnew(ii)<< " ";} std::cout << std::endl; 
        // std::cout << "clipper: deltaF: " << deltaF << std::endl; 
        // std::cout << "clipper: -params_.eps: " << -params_.eps << std::endl;  
        

        if (deltaF < -params_.eps) {
          // objective value decreased---we need to backtrack, so reduce step size
          // std::cout << "objective value decreased---we need to backtrack" << std::endl; 
          alpha = alpha * params_.beta;
        } else {
          // std::cout << "obj value increased, stop line search\n" << std::endl;
          break; // obj value increased, stop line search
        }
      ...
      // const double deltau = (unew - u).norm(); // l2 norm
      const double deltau = (unew.array()-u.array()).abs().maxCoeff(); // l1 norm

      #ifdef DEBUG_OPTIM
        // std::cout << "clipper: deltau: " << deltau << std::endl;
      #endif
    ...
    #ifdef DEBUG_OPTIM
      // std::cout << "clipper: largest change in u elements: " << deltau_elem << std::endl;
    #endif
    // if (deltau_elem < params_.tol_uout) break;
  ...
  // Generate output
  std::vector<int> nodes; // node indices of rounded u vector
  ...
  nodes = utils::findIndicesWhereAboveThreshold(u, rounding_thresh);
    ...
    for (int i : nodes) {std::cout << i << " ";} std::cout << std::endl;
  #endif
  #ifdef DEBUG_TIMING
      const auto t4 = std::chrono::high_resolution_clock::now(); // timer
      dur = std::chrono::duration_cast<std::chrono::microseconds>(t4 - t3);
...
{
  // std::cout << "running findMaximalClique_old ..." << std::endl;
  const auto t_start = std::chrono::high_resolution_clock::now();
  ...
  double F = 0; // objective value
  size_t i, j, k; // iteration counters
  size_t jsum=0, ksum=0; // total iteration counters
  ...
  for (i=0; i<params_.maxoliters; ++i) { // outerloop
    ...
    for (j=0; j<params_.maxiniters; ++j) { // innerloop
      ...
      for (k=0; k<params_.maxlsiters; ++k) { // line search
        ...
        // // safety procedure: make sure that at least one element of unew is > 0 (usually not entered)
        // if (unew.sum()==0) {
        //   const Eigen::VectorXi idx_grad_neg = (gradF.array() < 0).cast<int>();
        //   num = utils::selectFromIndicator(u, idx_grad_neg);
        //   den = utils::selectFromIndicator(gradF, idx_grad_neg);
        //   alpha = (num.array() / den.array()).abs().mean();
        //   unew = u + alpha * gradF;                     // gradient step
        //   unew = unew.cwiseMax(0);                      // project onto positive orthant
        // }
