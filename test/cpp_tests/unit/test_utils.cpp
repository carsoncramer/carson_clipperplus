...
TEST(AdjacencyMatrixToList, SmallGraph) {
    ...
    adj << 0, 1, 1,
           ...
           1, 1, 0;
    const int nnodes = 3;
    const long long nedges = 3;
    std::vector<int> ei, ej;

    clipperplus::adjmat_to_adjlist(adj, nnodes, nedges, ei, ej);
...
TEST(AdjacencyMatrixToList, InvalidInputs) {
    ...
    adj << 0, 1, 0,
           1, 0, 1;
    const int nnodes = 2;
    const long long nedges = 1;
    std::vector<int> ei, ej;

    ASSERT_THROW(clipperplus::adjmat_to_adjlist(adj, nnodes, nedges, ei, ej), std::invalid_argument);
}
