...
if (nrhs == 1) {
  if (mxIsSparse(prhs[0])) {
    ...
    mexErrMsgIdAndTxt("pmcheu:nargin", "adj must be sparse or full matrix.");
  }
} else {
  mexErrMsgIdAndTxt("pmcheu:nargin", "function only takes the adjacency matrix as input.");
}
...
if (nlhs > 3) {
  mexErrMsgIdAndTxt("pmcheu:nargout", "Only up to 3 output args supported (clq_size, clq, t)");
}
